...
function MonthGridSelector({
  ...
  selectedDate: Date;
  onSelect: (period: PeriodData) => void;
}) {
  ...
  const handleNavigation = (direction: 'prev' | 'next') => {
    setCurrentViewDate(
      direction === 'next' ? addYears(currentViewDate, 1) : subYears(currentViewDate, 1)
    );
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <h3 className="text-lg font-semibold text-white">{format(currentViewDate, 'yyyy')}</h3>
        <button
        ...
        {periods.map(period => (
          ...
          >
            <div className="text-center">
              <div className="text-sm font-medium text-white mb-1">{period.label}</div>
              <div className="text-xs text-white/70">₹{period.value.toLocaleString()}</div>
              {period.isActive && (
...
export default function PeriodSelector({ isOpen, onClose, onPeriodSelect }: PeriodSelectorProps) {
  ...
  return (
    <BottomSheet isOpen={isOpen} onClose={onClose} title="Select Month" maxHeight="max-h-[80vh]">
      <MonthGridSelector selectedDate={new Date()} onSelect={handlePeriodSelect} />
    </BottomSheet>
  );
}
